...
function TasteMatcher({ addToCart }: TasteMatcherProps) {
  ...
  const getResult = () => {
    if (answers.q1 === 'gurih') {
      return { 
        name: "Kastengel Royal", 
        desc: "Gurihnya keju edam asli, renyah di luar lumer di dalam.", 
        price: 95000 
      };
    }
    if (answers.q1 === 'manis' && answers.q3 === 'lumer') {
      return { 
        name: "Nastar Classic", 
        desc: "Selai nanas homemade yang lumer berpadu dengan adonan butter premium.", 
        price: 85000 
      };
    }
    return { 
      name: "Almond Crispy", 
      desc: "Tipis, renyah, manis yang pas buat nemenin maraton series favoritmu.", 
      price: 65000 
    };
  };
          ...
          >
            <div className="flex items-center gap-2 mb-6">
              {[1, 2, 3].map((num) => (
                <div 
                  key={num} 
                  className={`h-2 flex-1 rounded-full border-2 border-[#4A3022] transition-all duration-500 ${num <= step ? 'bg-[#D97736] scale-y-110' : 'bg-white'}`} 
                />
              ))}
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {questions[step - 1].options.map((opt, idx) => (
                ...
                >
                  ...
                  <span className="font-jakarta font-black text-[#4A3022] group-hover:text-white transition-colors">
                    {opt.label}
                  </span>
                </motion.button>
              ))}
                ...
                <>
                  ...
                  <div className="pt-4 flex flex-col gap-3">
                    <button 
                      onClick={() => {
                        if (addToCart) {
                          addToCart({ id: `kuis-${res.name}`, name: res.name, price: res.price });
                        }
                        setStep(0);
                        setAnswers({});
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-16 md:pt-24 pb-20 flex flex-col-reverse md:flex-row items-center gap-12 overflow-hidden min-h-[85vh]">
        <div className="w-full md:w-1/2 space-y-8 relative z-10">
          ...
          >
            <motion.button 
              whileHover={{ y: -4 }} 
              whileTap={{ y: 2, boxShadow: "0px 0px 0px rgba(74,48,34,1)" }} 
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          >
            ...
            <div className="relative z-10 w-full md:w-1/2 bg-white p-6 rounded-3xl border-4 border-[#4A3022] shadow-[6px_6px_0px_rgba(74,48,34,1)] mt-auto">
              <div className="inline-block px-3 py-1 bg-[#D97736] text-[#FAF5E9] rounded-full text-xs font-jakarta font-black mb-3 border-2 border-[#4A3022]">
                BEST SELLER
              </div>
              <h3 className="text-3xl font-playfair font-black text-[#4A3022] mb-2">
                Kastengel Royal
              </h3>
              <p className="text-[#4A3022]/80 font-jakarta font-bold mb-4 text-sm leading-relaxed">
                Garing di luar, keju edam yang pecah dan lumer di gigitan pertama.
              </p>
          ...
          <div className="w-full md:w-1/2 grid grid-cols-2 gap-6 relative">
            <motion.div style={{ y: yParallax1 }} className="pt-20">
              <div className="bg-white rounded-[2rem] flex flex-col overflow-hidden border-4 border-[#4A3022] shadow-[8px_8px_0px_#4A3022] hover:-translate-y-2 transition-transform cursor-pointer">
                <div className="h-40 sm:h-48 w-full border-b-4 border-[#4A3022]">
                  <img 
                    src="https://i.pinimg.com/1200x/8b/ca/86/8bca86af3169e15ef1dcb7a5ffd150b6.jpg" 
                    alt="Premium Butter" 
            ...
            <motion.div style={{ y: yParallax2 }}>
              <div className="bg-white rounded-[2rem] flex flex-col overflow-hidden border-4 border-[#4A3022] shadow-[8px_8px_0px_#4A3022] hover:-translate-y-2 transition-transform cursor-pointer">
                <div className="h-40 sm:h-48 w-full border-b-4 border-[#4A3022]">
                  <img 
                    src="https://i.pinimg.com/736x/06/a5/25/06a525f631c5e2d058b06c15228eb296.jpg" 
                    alt="Keju Edam Asli" 
                    className="w-full h-full object-cover" 
                  />
                </div>
          ...
          {[
            ...
            >
              <div>
                ...
                <p className="text-[#4A3022] font-jakarta font-bold text-lg mb-8 leading-relaxed">
                  "{review.text}"
                </p>
              </div>
              <div className="flex items-center gap-4 border-t-4 border-[#4A3022] pt-6">
                ...
                <div>
                  ...
                  <p className="text-xs font-jakarta text-[#4A3022]/70 font-bold uppercase tracking-wider mt-1">
                    {review.role}
                  </p>
                </div>
              </div>
