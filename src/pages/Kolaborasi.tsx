...
export default function Kolaborasi() {
  ...
  return (
    ...
    >
      {/* QUIRKY TILTED MARQUEE (Ala Tuku/Indie Brand) */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-48 md:pt-56 relative z-10">
        ...
        {/* CARDS (GAYA TIKET RETRO) */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8 mb-16">
          {collabOptions.map((opt, idx) => (
            <motion.button
              key={opt.id}
              initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.1, type: "spring" }}
              onClick={() => {
                setSelectedType(opt.id);
                // Auto scroll ke bawah (ke form)
            ...
            >
              <div className={`h-full border-4 border-[#4A3022] rounded-[2rem] p-6 bg-white flex flex-col z-10 relative transition-all ${
                selectedType === opt.id ? 'shadow-[8px_8px_0px_0px_#D97736]' : 'shadow-[8px_8px_0px_0px_#4A3022] hover:shadow-[12px_12px_0px_0px_#4A3022]'
              }`}>
                <div className={`w-16 h-16 rounded-2xl flex items-center justify-center text-white mb-6 border-2 border-[#4A3022] shadow-[4px_4px_0px_0px_#4A3022] ${opt.color} group-hover:scale-110 transition-transform duration-300`}>
                  {opt.icon}
                </div>
        ...
        <AnimatePresence mode="wait">
          {selectedType && (
            ...
            >
              <form onSubmit={handleSendWA} className="bg-[#FAF5E9] p-8 md:p-12 rounded-[3rem] border-4 border-[#4A3022] shadow-[12px_12px_0px_0px_#4A3022] relative z-10 mb-8">
                
                {/* Aksen Solasi / Tape di atas form */}
                ...
                <div className="mb-10 text-center">
                  ...
                  <p className="text-[#4A3022]/70 font-bold">Isi detailnya, biar kita bisa ngobrol lebih enak.</p>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
                  <div className="space-y-2">
                    <label className="text-sm font-black text-[#4A3022] uppercase tracking-wider">Nama Kamu *</label>
                    <input required name="name" onChange={handleInputChange} type="text" className="w-full bg-white border-4 border-[#4A3022] rounded-xl p-4 outline-none text-[#4A3022] font-bold focus:shadow-[4px_4px_0px_0px_#D97736] focus:-translate-y-1 transition-all" placeholder="Panggil aja..." />
                  </div>
                  ...
                  {selectedType === 'corporate' && (
                    <>
                      <div className="space-y-2">
                        ...
                        <input required name="company" onChange={handleInputChange} type="text" className="w-full bg-white border-4 border-[#4A3022] rounded-xl p-4 outline-none text-[#4A3022] font-bold focus:shadow-[4px_4px_0px_0px_#D97736] focus:-translate-y-1 transition-all" placeholder="PT Sukses Selalu" />
                      </div>
                      <div className="space-y-2">
                        <label className="text-sm font-black text-[#4A3022] uppercase tracking-wider">Estimasi Jumlah Box</label>
                        <input name="quantity" onChange={handleInputChange} type="text" className="w-full bg-white border-4 border-[#4A3022] rounded-xl p-4 outline-none text-[#4A3022] font-bold focus:shadow-[4px_4px_0px_0px_#D97736] focus:-translate-y-1 transition-all" placeholder="Kira-kira berapa?" />
                      </div>
                      <div className="space-y-2">
                        <label className="text-sm font-black text-[#4A3022] uppercase tracking-wider">Tanggal Acara</label>
                        <input name="date" onChange={handleInputChange} type="date" className="w-full bg-white border-4 border-[#4A3022] rounded-xl p-4 outline-none text-[#4A3022] font-bold focus:shadow-[4px_4px_0px_0px_#D97736] focus:-translate-y-1 transition-all" />
                      </div>
                  ...
                  {selectedType === 'cafe' && (
                    <>
                      <div className="space-y-2">
                        ...
                        <input required name="company" onChange={handleInputChange} type="text" className="w-full bg-white border-4 border-[#4A3022] rounded-xl p-4 outline-none text-[#4A3022] font-bold focus:shadow-[4px_4px_0px_0px_#D97736] focus:-translate-y-1 transition-all" placeholder="Teras Kopi..." />
                      </div>
                      <div className="space-y-2">
                        <label className="text-sm font-black text-[#4A3022] uppercase tracking-wider">Butuh Berapa per Hari?</label>
                        <input name="quantity" onChange={handleInputChange} type="text" className="w-full bg-white border-4 border-[#4A3022] rounded-xl p-4 outline-none text-[#4A3022] font-bold focus:shadow-[4px_4px_0px_0px_#D97736] focus:-translate-y-1 transition-all" placeholder="Contoh: 30 Croissant" />
                      </div>
                    </>
                  )}
                  ...
                  {selectedType === 'collab' && (
                    <div className="space-y-2 md:col-span-2">
                      ...
                      <input required name="company" onChange={handleInputChange} type="text" className="w-full bg-white border-4 border-[#4A3022] rounded-xl p-4 outline-none text-[#4A3022] font-bold focus:shadow-[4px_4px_0px_0px_#D97736] focus:-translate-y-1 transition-all" placeholder="Brand / Acara Lo..." />
                    </div>
                  )}

                  <div className="space-y-2 md:col-span-2">
                    <label className="text-sm font-black text-[#4A3022] uppercase tracking-wider">Ceritain idemu ke kita *</label>
                    <textarea required name="message" onChange={handleInputChange} rows={4} className="w-full bg-white border-4 border-[#4A3022] rounded-xl p-4 outline-none text-[#4A3022] font-bold focus:shadow-[4px_4px_0px_0px_#D97736] focus:-translate-y-1 transition-all resize-none" placeholder="Halo Oriena, gw ada ide nih..."></textarea>
                  </div>
                </div>
