...
export default function HampersBuilder({ setCartCount }: HampersBuilderProps) {
  ...
  return (
    <motion.div 
      ...
      animate={{ opacity: 1 }} 
      exit={{ opacity: 0 }} 
      className="w-full bg-[#F7F3EB]"
    >
      
      {/* Section: Hero & Builder Layout */}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
            ...
            >
              <div className="w-full max-w-lg flex justify-between items-center mb-6 px-2">
                <h3 className="text-2xl font-playfair font-black text-[#4A3022] bg-[#FAF5E9] px-3 py-1 rounded-lg border-2 border-[#4A3022]">
                  Box Pengemasan
                </h3>
                  ...
                  {[0, 1, 2].map((slotIdx) => (
                    <div 
                      ...
                      className={`relative rounded-full flex items-center justify-center overflow-hidden transition-all duration-300 w-[140px] h-[140px] md:w-[170px] md:h-[170px] ${
                        boxItems[slotIdx] 
                          ? 'bg-transparent' 
                          : 'bg-[#FAF5E9] border-4 border-dashed border-[#4A3022] hover:border-[#D97736] hover:bg-white'
                      ...
                      <AnimatePresence mode="wait">
                        {boxItems[slotIdx] ? (
                          <motion.div 
                            ...
                            className="w-full h-full relative group rounded-full overflow-hidden border-4 border-[#4A3022]"
                          >
                            <img 
                              src={boxItems[slotIdx]!.image} 
                              alt={boxItems[slotIdx]!.name} 
                            ...
                            <div className="absolute inset-0 bg-gradient-to-t from-[#4A3022] via-[#4A3022]/40 to-transparent flex flex-col justify-end p-2 md:p-4 opacity-90">
                              <span className="font-bold font-jakarta text-[#FAF5E9] text-[10px] md:text-sm text-center leading-tight">
                                {boxItems[slotIdx]!.name}
                              </span>
                            </div>
                            <div 
                              onClick={() => handleRemoveCookie(slotIdx)} 
                              className="absolute inset-0 bg-[#4A3022]/80 opacity-0 group-hover:opacity-100 flex items-center justify-center cursor-pointer transition-opacity duration-300"
                            >
                          ...
                          <motion.div 
                            ...
                            initial={{ opacity: 0 }} 
                            animate={{ opacity: 1 }} 
                            className="text-[#4A3022]/30 font-playfair font-black text-3xl md:text-5xl"
                          >
                            {slotIdx + 1}
                          </motion.div>
                        )}
                ...
                <AnimatePresence>
                  {isDone && (
                    ...
                    >
                      ...
                      <span className="text-white font-playfair font-black text-3xl tracking-widest uppercase">
                        Box Terisi Penuh
                      </span>
                    </motion.div>
                  )}
            ...
            {/* Section: Cookie Selection (Right) */}
            <motion.div 
              ...
              <AnimatePresence mode="wait">
                {!isDone ? (
                  <motion.div 
                    ...
                    className="bg-[#FAF5E9] rounded-[3rem] p-8 border-4 border-[#4A3022] shadow-[8px_8px_0px_rgba(74,48,34,1)]"
                  >
                    <div className="mb-6">
                      ...
                      <p className="text-sm font-jakarta text-[#4A3022]/80 font-medium ml-14">
                        Klik tombol + atau seret kue ke dalam box.
                      </p>
                    </div>
                        ...
                        return (
                          <motion.div
                            ...
                            className={`w-full flex items-center justify-between p-3 rounded-2xl border-4 transition-all ${
                              isFull 
                                ? 'opacity-50 cursor-not-allowed border-[#4A3022]/20 bg-[#E0D0BB]' 
                                : 'border-[#4A3022] hover:border-[#D97736] bg-white cursor-grab active:cursor-grabbing shadow-[4px_4px_0px_#4A3022] hover:shadow-[2px_2px_0px_#D97736]'
                            }`}
                          >
                            ...
                            <button 
                              ...
                              className="w-10 h-10 rounded-full flex items-center justify-center text-[#4A3022] border-2 border-[#4A3022] hover:bg-[#D97736] hover:text-white hover:border-[#D97736] transition-colors bg-[#FAF5E9]"
                            >
                  ...
                  <motion.div 
                    ...
                    animate={{ opacity: 1, scale: 1 }} 
                    exit={{ opacity: 0, scale: 0.95 }} 
                    className="bg-[#FAF5E9] rounded-[3rem] p-8 border-4 border-[#4A3022] shadow-[8px_8px_0px_#4A3022] relative overflow-hidden"
                  >
                    <div className="mb-8 flex items-center gap-3 relative z-10">
                      <div className="w-12 h-12 bg-[#829079] border-2 border-[#4A3022] text-white rounded-full flex items-center justify-center shadow-[4px_4px_0px_#4A3022]">
                        <CheckCircle size={28} />
                      </div>
                      <div>
                    ...
                    <div className="bg-white border-4 border-[#4A3022] rounded-2xl p-6 mb-8 relative z-10 shadow-[4px_4px_0px_#4A3022]">
                      <h4 className="font-jakarta font-black text-[#4A3022] mb-4 border-b-2 border-dashed border-[#4A3022] pb-3 text-lg uppercase tracking-wider">
                        Estimasi Biaya
                      </h4>
                      <div className="flex justify-between items-center mb-4">
                        <span className="font-jakarta font-bold text-[#4A3022]/80">Total Harga Produk</span>
                        <span className="font-jakarta font-black text-[#4A3022] text-lg">
                          Rp {currentTotal.toLocaleString('id-ID')}
                        </span>
                      </div>
                      <div className="flex justify-between items-center bg-[#F7F3EB] p-3 rounded-xl border-2 border-[#4A3022]">
                        <span className="font-jakarta font-bold text-[#4A3022]/80">Harga Box Kemasan</span>
                        <span className="font-jakarta font-black text-[#D97736]">Rp 6.000 - 100.000</span>
                      </div>
                      <p className="text-xs font-jakarta font-bold text-[#4A3022]/60 mt-4 leading-relaxed bg-white border-l-4 border-[#D97736] pl-3 py-1">
                        *Detail biaya box dan ongkir akan dibantu hitung oleh admin kami via WhatsApp.
                      </p>
                    </div>

                    <div className="space-y-4 relative z-10">
                      <motion.button 
                        onClick={handleHampersWA} 
                        whileHover={{ scale: 1.02 }} 
                        whileTap={{ scale: 0.95 }} 
                        className="w-full text-white bg-[#D97736] py-4 border-4 border-[#4A3022] rounded-2xl font-jakarta font-black text-lg flex items-center justify-center gap-2 shadow-[4px_4px_0px_#4A3022]"
                      >
                        <MessageSquare size={24} /> Pesan Custom via WA
                      </motion.button>
                      <button 
                        onClick={() => setIsDone(false)} 
                        className="w-full py-3 text-[#4A3022] font-jakarta font-black text-sm hover:text-[#D97736] hover:bg-white border-4 border-transparent hover:border-[#D97736] rounded-xl transition-all"
                      >
      ...
      <section className="py-24 bg-[#F7F3EB] px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16">
            <h2 className="text-4xl md:text-5xl font-playfair font-black text-[#4A3022] mb-4">
              Cara Kerja Hampers Custom
            </h2>
            <p className="text-lg font-jakarta font-bold text-[#4A3022]/80">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8 relative">
            ...
            {[
              { title: "Pilih Isian", desc: "Tarik dan lepas 3 varian kue favoritmu ke dalam box pengemasan.", icon: <Cookie size={32} /> },
              { title: "Cek Estimasi", desc: "Lihat total harga isi produk dan perkiraan harga box yang tersedia.", icon: <PenLine size={32} /> },
              ...
              >
                ...
                <h3 className="text-xl font-playfair font-black text-[#4A3022] mb-2">
                  Langkah {idx + 1}: <br/> {step.title}
                </h3>
                <p className="text-sm font-jakarta font-medium text-[#4A3022]/80">
                  {step.desc}
                </p>
              </motion.div>
            ))}
